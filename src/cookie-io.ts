...
export class CookieIO {
    ...
    public static set<T>(name: string, value: T, options: CookieOptions = {path: '/', session: true}): void {
        let cookieData = ''
        ...
        if (options.session) {
            ...
            const expiry = options.expiry ? options.expiry.toUTCString() : new Date(new Date().getTime() + (365 * 24 * 60 * 60 * 1000)).toUTCString()
