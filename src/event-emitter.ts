...
export type EventListenerOptions = {
    once?: boolean; signal?: AbortSignal;
}
...
export class EventEmitter<T = EventEmitterDefault> {
    ...
    addEventListener<E extends keyof T>(eventName: E, listener: EmitterEventListener<T[E]>, options?: EventListenerOptions) {
        this.on(eventName, listener, options);
    }

    removeEventListener<E extends keyof T>(eventName: E, listener: EmitterEventListener<T[E]>) {
        this.off(eventName, listener);
    }

    on<E extends keyof T>(eventName: E, listener: EmitterEventListener<T[E]>, options?: EventListenerOptions) {
    ...
    once<E extends keyof T>(eventName: E, listener: EmitterEventListener<T[E]>, options?: Omit<EventListenerOptions, "once">) {
        if (!(eventName in this.callbacks)) {
            this.callbacks[eventName] = new Set();
        }
        this.callbacks[eventName].add({
    ...
    protected trigger<E extends keyof T>(eventName: E, event: T[E]) {
        if (eventName in this.callbacks) {
            ...
            if (callbacks) {
                callbacks.forEach(item => {
                    if (item.options && item.options.signal && item.options.signal.aborted) {
                        callbacks.delete(item)
                    } else {
                        item.listener(event);
                        if (item.options) {
